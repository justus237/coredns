...
func (h *cryptoSetup) GetSessionTicket() ([]byte, error) {
	...
	ret_b, err := h.conn.GetSessionTicket(appData)

	file, _ := os.OpenFile("/tmp/coredns_session_stuff.txt", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	datawriter := bufio.NewWriter(file)
	_, _ = datawriter.WriteString(fmt.Sprintf("get session ticket called: %x", ret_b) + "\n")
	datawriter.Flush()
	file.Close()
	return ret_b, err
}
...
func (h *cryptoSetup) accept0RTT(sessionTicketData []byte) bool {
	file, _ := os.OpenFile("/tmp/coredns_session_stuff.txt", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	datawriter := bufio.NewWriter(file)
	_, _ = datawriter.WriteString(fmt.Sprintf("received session ticket from client: %x", sessionTicketData) + "\n")
	datawriter.Flush()
	file.Close()
	var t sessionTicket
